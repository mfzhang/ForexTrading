...
void StrategySwarm::updatePSOSwarm(vector<Particle *> swarmParams) {
    ...
    for(int i=0;i<swarmSize;i++)
    {
        ...
        {
            ////Update Velocity
            omega1=swarmParams.at(j)->parameters.at(3)*r1.at(j);
            omega2=swarmParams.at(j)->parameters.at(4)*r2.at(j);
            omega=omega1+omega2;

            X=2*(swarmParams.at(j)->parameters.at(5))/(2-(omega)-(sqrt(abs(omega*(omega-4)))));
